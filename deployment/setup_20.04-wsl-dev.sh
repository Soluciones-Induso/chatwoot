...
apt install -y \
	software-properties-common imagemagick libpq-dev \
    libxml2-dev libxslt1-dev file g++ gcc autoconf build-essential \
    libssl-dev libyaml-dev libreadline-dev gnupg2 redis-server \
    redis-tools postgresql postgresql-contrib \
    yarn patch ruby-dev zlib1g-dev liblzma-dev \
...
echo "¡En hora buena! La instalación del servidor Chatwoot ha completado"
echo "Para ejecutar el servidor ejecute `RAILS_ENV=development rails server`"
echo "El servidor se puede acceder en http://localhost:3000"
